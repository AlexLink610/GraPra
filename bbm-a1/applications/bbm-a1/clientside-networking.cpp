...
extern std::shared_ptr<Skysphere> the_skysphere;
...
void send_message(const msg::message& msg) {
	try {
...
void client_message_reader::handle_message(msg::board_info* m) {
    ...
    the_skysphere = std::make_shared<Skysphere>(tiles_x, tiles_y, 2 * max(tiles_x, tiles_y));
}
